...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnon) {
      return NextResponse.json({ error: "Supabase not configured", requestId }, { status: 500, headers: { "x-request-id": requestId } });
    }
    ...
    if (!clientId) {
      return NextResponse.json(
        { error: "Missing X_CLIENT_ID", requestId },
    ...
    res.cookies.set("x_supabase_token", auth.accessToken, {
      httpOnly: true,
      sameSite: "lax",
      secure,
      path: "/",
    ...
    console.error("[integrations:x:start]", { requestId, message });
    return NextResponse.json(
