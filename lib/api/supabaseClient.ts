...
let inFlightSessionPromise: Promise<{ session: Session | null; timedOut: boolean }> | null = null;
let sessionCache: { session: Session; cachedAtMs: number } | null = null;
...
export async function getSessionWithTimeout(opts?: {
  ...
  if (signal?.aborted) return { session: null, timedOut: false };

  const usable = (s: Session | null) => Boolean(s && !isExpiredOrNearExpiry(s.expires_at, 60));

  if (sessionCache) {
    if (usable(sessionCache.session)) {
      return { session: sessionCache.session, timedOut: false };
    }
    sessionCache = null;
  }

  const storedSession = tryReadStoredSessionSnapshot();
  if (storedSession && usable(storedSession)) {
    sessionCache = { session: storedSession, cachedAtMs: Date.now() };
    return { session: storedSession, timedOut: false };
  }

  const waitAbort = (sig: AbortSignal) =>
    new Promise<{ kind: "abort" }>((resolve) => {
      if (sig.aborted) return resolve({ kind: "abort" });
      sig.addEventListener("abort", () => resolve({ kind: "abort" }), { once: true });
    });

  const awaitInFlight = async () => {
    if (!inFlightSessionPromise) return null;
    if (!signal) return await inFlightSessionPromise;
    const raced = await Promise.race([inFlightSessionPromise, waitAbort(signal)]);
    if ((raced as { kind?: string }).kind === "abort") return { session: null, timedOut: false };
    return raced as { session: Session | null; timedOut: boolean };
  };

  const existing = await awaitInFlight();
  if (existing) {
    if (existing.session && usable(existing.session)) {
      sessionCache = { session: existing.session, cachedAtMs: Date.now() };
    }
    return existing;
  }
  ...
  inFlightSessionPromise = (async () => {
    ...
    const snapshotRetries = Math.min(3, Math.max(0, retries));
    for (let attempt = 0; attempt <= snapshotRetries; attempt++) {
      const s = tryReadStoredSessionSnapshot();
      if (s && usable(s)) return { session: s, timedOut: anyTimeout };
      if (attempt < snapshotRetries) await sleep(Math.max(0, retryDelayMs));
    }
    ...
    if (raced.kind === "timeout") {
      ...
      const storedAfter = tryReadStoredSessionSnapshot();
      if (storedAfter && usable(storedAfter)) {
    ...
    if (usable(session)) return { session, timedOut: anyTimeout };
    ...
    if (session && isExpiredOrNearExpiry(session.expires_at, 60)) {
      ...
      if (refreshed.kind === "timeout") {
        ...
        if (usable(refreshedSession)) return { session: refreshedSession, timedOut: anyTimeout };
      }
    }
    ...
    .then((result) => {
      if (result.session && usable(result.session)) {
        sessionCache = { session: result.session, cachedAtMs: Date.now() };
      }
      return result;
    })
    .finally(() => {
  ...
  const result = await (signal ? Promise.race([inFlightSessionPromise, waitAbort(signal)]) : inFlightSessionPromise);
  if ((result as { kind?: string }).kind === "abort") return { session: null, timedOut: false };
  return result as { session: Session | null; timedOut: boolean };
}
